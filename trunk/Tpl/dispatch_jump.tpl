...
<div class="message">
	...
	<present name="closeWin" >
		Trình duyệt sẽ tự động chuyển sau <span class="wait">{$waitSecond}</span> giây nữa, nếu bạn không muốn chờ <a href="{$jumpUrl}">nhấn vào đây</a>.
	<else/>
		Trình duyệt sẽ tự động chuyển sau  <span class="wait">{$waitSecond}</span> giây nữa, nếu bạn không muốn chờ <a href="{$jumpUrl}">nhấn vào đây</a>.
	</present>
	</div>
