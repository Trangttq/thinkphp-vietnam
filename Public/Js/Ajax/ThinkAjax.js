...
var ThinkAjax = {
	...
	{
		var _self = this;
	   $(source).addEvent(event,function (){_self.send(url,vars,response)});
	},
	// 页面加载完成后执行Ajax操作
	load:function(url,vars,response)
	{
		var _self = this;
	   window.addEvent('load',function (){_self.send(url,vars,response)});
	},
	// 延时执行Ajax操作
	time:function(url,vars,time,response)
	{
		var _self = this;
		myTimer =  window.setTimeout(function (){_self.send(url,vars,response)},time);
	},
	// 定制执行Ajax操作
	repeat:function(url,vars,intervals,response)
	{
		...
		myTimer = window.setInterval(function (){_self.send(url,vars,response)},intervals);
	}
}
